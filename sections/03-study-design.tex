...
%\textbf{RQ2}: \textit{Do an aided approach of code smell detection - using a tree classifier representation – increase the confidence of developers during each detection?}

%The motivation for this question aims at analysing if the metric-based rules from decision tree classifier makes the developers more confident during evaluations of code smell on suspicious source code. The fact of showing more elements than a simple "raw" code, such elements being rules that indicates the presence of code smell, may  offer to the developer new insights that he didn't know or notice before. Thus, it would be natural that equipped with more elements to decision making, he makes assignments with more confidence.
...
\subsection{Training the algorithm and generating the classifiers}
